...
- **桌面设置**：WebUI 开关、可选固定端口（带冲突预检）、丰富的关于信息——全部外壳层实现，不改 DSH
- **应用升级**：NSIS 安装版从 GitHub Release 自动更新（便携版 / 未签名 macOS / deb 降级为打开下载页）
...
- **只做外壳层设置**：WebUI 开关 / 固定端口保存在外壳自己的
  `userData/settings.json`，通过 DSH 官方参数（`--host 127.0.0.1 --port <n>`）透传，
  不写 DSH_HOME、不动 profile 与 composition。局域网开放（`--host 0.0.0.0`）因上游
  出于安全明确拒绝而保持禁用——远程访问请用 SSH 隧道。
...
| `<数据目录>\settings.json` | 外壳设置（WebUI 开关、固定端口） |
| `<数据目录>\server.log` | 主进程 + DSH 服务日志（含 `[main ...]` 行） |

右键窗口标题栏可弹出菜单（含设置入口）；「帮助」菜单里可直接打开数据目录与日志；
出错时窗口内的错误页也会给出日志路径。

**从官方原版迁移对话数据**（只复制、不影响原版）：见 [docs/MIGRATION.md](docs/MIGRATION.md)。
...
- CI（GitHub Actions）三平台矩阵构建（Windows/macOS/Linux）：push 触发构建，
  打 `v*` tag 自动发布 Release（含全平台安装包与自动更新用的 `latest*.yml` 元数据）。

## ❓ 常见问题

- **双击后窗口里一直是加载页**：首次启动需解压运行时并初始化 DSH，通常 1–3 分钟。
  若超过 3 分钟，窗口会显示错误与日志；可右键标题栏 →「重试启动」。
...
│   ├── preload.cjs       # 设置页 IPC 桥（contextBridge）
│   ├── settings.cjs      # 外壳设置存储（userData/settings.json）
│   ├── control.html      # 设置页（DSH 设计令牌风格）
│   └── loading.html      # 启动加载页
├── scripts/
│   ├── gen-icon.mjs      # 生成应用图标（纯 Node，无依赖）
│   ├── fetch-electron.mjs# 带重试的 electron 二进制下载脚本（GitHub→npmmirror 回退）
│   └── patch-picker-worker.mjs # koffi/Electron 目录选择器崩溃的 postinstall 补丁
├── docs/FEATURES.md      # 桌面功能兼容性调研
